...
int main(int argc, char **argv) 
{
  ...
  if (argc < 5) {
    cerr << "Usage: " << argv[0] << " <bag file> <camera_name> <parameter_file> <tf_file> [max_images]\n";
    return -1;
  }
  ...
  try {
    ...
    ifs >> T.matrix()(0,3) >> T.matrix()(1,3) >> T.matrix()(2,3); 
  } catch (exception &e) {
    cerr << "Could not load tf file";
    return -1;
  }
  std::cout << "T:\n" << T.matrix() << endl;
  
  if (argc > 5) {
    max_cont = stoi(argv[5]);
  }
  ...
  FloorDetector p(delta, epsilon, gamma, theta, camera, T);
